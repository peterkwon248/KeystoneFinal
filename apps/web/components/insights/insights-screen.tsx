// source/Insights.jsx InsightsView 이식 — 여러 플랜을 가로지르는 순수 분석 뷰(과정 측정).
// 섹션: 시나리오 적중률 · 성과(관점 vs 전략) · 프로세스 건강도 · 승률/손익.
// 대시보드와 동일 성격(새 DB 테이블 없음, plans 파생). t/lang 은 usePrefs + I18N[lang]로 획득.
// 단일 플랜 스코프(sel)는 이미 이식된 plan 상세 인사이트 탭(InsightsTab)을 그대로 재사용.
// 어댑테이션: React.useState → named useState, 전역 STRATEGIES/EXEC_STRATEGIES/planReturn → core import,
//   전역 scLabel/insMonthIdx → t["case"+key] 폴백 / trajMonthIdx, StatusIcon/Lic → @/components/icons.
// SWC 함정 회피: JSX 안 제네릭 앵글브래킷 캐스트 없음. SVG 좌표 계산은 return 이전 함수 본문에서 수행.
"use client";
...
import { StatusIcon, Lic } from "@/components/icons";
...
import { InsightsTab } from "@/components/plan/insights-tab";
...
function insMonthIdx(d: string | null | undefined): number {
  const v = trajMonthIdx(d);
  return v == null ? 0 : v;
}
...
function PerfScatter({ rows, ko }: { rows: PerfRow[]; ko: boolean }) {
  ...
  const X = (wr: number | null) => padL + (Math.max(0, Math.min(100, wr || 0)) / 100) * iw;
  const Y = (av: number) => padT + (1 - (av + maxAbs) / (2 * maxAbs)) * ih;
  const maxCount = Math.max(1, ...rows.map((r) => r.count));
  const rad = (c: number) => 5 + (c / maxCount) * 7;
  const y0 = Y(0), x50 = X(50);
  void y0;
  // collision-aware layout: relax overlapping dots vertically within same-x clusters
  const laid = rows.filter((r) => r.avg != null).map((r) => ({ r, cx: X(r.winRate), rr: rad(r.count), cy: Y(r.avg as number) }));
  const clusters: Record<number, typeof laid> = {};
    ...
    <div className="ins-scatter">
      ...
      <div className="ins-sc-overlay">
        {laid.map(({ r, cx, cy }) => {
          ...
          const tx: React.CSSProperties = fx > 0.6 ? { right: 0, left: "auto" } : fx < 0.4 ? { left: 0, right: "auto" } : { left: "50%", transform: "translateX(-50%)" };
          const ty: React.CSSProperties = fy < 0.45 ? { top: 13 } : { bottom: 13 };
...
/* ---- 1. Scenario accuracy ---- */
function ScenarioAccuracy({ plans, t, lang }: { plans: UIPlan[]; t: I18nDict; lang: Lang }) {
  const ko = lang === "ko";
  const buckets: Record<string, { realized: number; invalidated: number; pending: number; gaps: number[] }> = {};
            ...
            {SCASE.map((c) => {
              ...
              return (
                <div className="ins-row" key={c.key}>
                  <span className="ins-row-label"><span className="scsum-dot" style={{ background: c.color }} />{t["case" + c.key]}</span>
...
function PerformanceSection({ plans, t, lang }: { plans: UIPlan[]; t: I18nDict; lang: Lang }) {
  const ko = lang === "ko";
  const [by, setBy] = useState<"lens" | "strat">("lens"); // lens(관점) | strat(전략)
  const defs = by === "lens"
    ? STRATEGIES.filter((s) => s.model).map((s) => ({ id: s.id, name: s.name, color: s.color, ps: plans.filter((p) => p.strategyId === s.id) }))
    : EXEC_STRATEGIES.map((s) => ({ id: s.id, name: s.name, color: s.color, ps: plans.filter((p) => p.execId === s.id) }));
  const rows: PerfRow[] = defs.map((g) => {
    const rets = g.ps.map((p) => planReturn(p)).filter((r): r is NonNullable<typeof r> => Boolean(r));
...
/* ---- 3. Process health ---- */
function ProcessHealth({ plans, t, lang }: { plans: UIPlan[]; t: I18nDict; lang: Lang }) {
  const ko = lang === "ko";
  const entryGaps: number[] = [], closeGaps: number[] = [];
  plans.forEach((p) => {
  ...
  const flow = [
    { k: t.toResearch, color: "var(--r-research)", dur: null as number | null },
...
/* ---- 4. Win / loss ---- */
function WinLoss({ plans, t, lang }: { plans: UIPlan[]; t: I18nDict; lang: Lang }) {
  const ko = lang === "ko";
...
function InsightsView({ plans, t, lang, onOpenPlan }: {
  plans: UIPlan[]; t: I18nDict; lang: Lang; onOpenPlan: (p: UIPlan) => void;
}) {
  const [scope, setScope] = useState<string>("all"); // "all" | plan id
  ...
  return (
    <div className="body-main">
      <div className="ins-wrap">
        ...
        {sel
          ? <InsightsTab plan={sel} t={t} lang={lang} />
          : <>
            ...
            <WinLoss plans={plans} t={t} lang={lang} />
          </>}
      </div>
    </div>
  );
...
export function InsightsScreen({ plans }: { plans: UIPlan[] }) {
  const router = useRouter();
  const { lang }: { lang: Lang } = usePrefs();
  const t = I18N[lang];
  const openPlan = (p: UIPlan) => router.push(`/plans/${p.dbId}`);
  return <InsightsView plans={plans} t={t} lang={lang} onOpenPlan={openPlan} />;
}
