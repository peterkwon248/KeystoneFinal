// source/Dashboard.jsx 이식본 — 운영 "현황" 뷰: 포트폴리오 롤업 + 플랜별 라이브 상태.
// 라운드/평단/투입/손익은 각 플랜 체결의 롤업 — 시장가만 mock. 로우는 체결 원장으로 딥링크.
// 어댑테이션: 전역 React 훅 → react import, securityOf 전역 → plan.sector 필드(plan-mapper),
// orderPlans 는 웹 group.tsx(코어엔 없음)에서 가져온다.
"use client";
import { useState, useRef, useLayoutEffect } from "react";
import type { I18nDict, Lang } from "@keystone/core/types";
import { EXEC_STRATEGIES, PORTFOLIOS, STATUS_ORDER } from "@keystone/core/reference";
import { planReturn, planTag } from "@keystone/core/analytics";
import { fmtCompact } from "@keystone/core/format";
import { Flag, StatusIcon, Lic } from "@/components/icons";
...
type TmapMode = "none" | "portfolio" | "sector" | "market";
interface TmapGroup { key: string; label: string; color: string }

// group a plan by the chosen key → { key, label, color }
function tmapGroupOf(p: UIPlan, mode: TmapMode, lang: Lang): TmapGroup {
  if (mode === "market") {
    const kr = p.cur === "KRW";
    return { key: kr ? "KR" : "US", label: kr ? (lang === "ko" ? "한국" : "Korea") : (lang === "ko" ? "미국" : "US"), color: kr ? "#4C8DFF" : "#BB6BD9" };
  }
  if (mode === "sector") {
    const s = p.sector ?? { en: "Other", ko: "기타" };
    return { key: s.en, label: s[lang] || s.en, color: "var(--fg-3)" };
  }
  // portfolio (default)
  const pf = PORTFOLIOS.find((x) => x.id === p.portfolioId);
  return pf ? { key: pf.id, label: pf.name[lang], color: pf.color } : { key: "none", label: lang === "ko" ? "미분류" : "Unassigned", color: "var(--fg-4)" };
}

interface GroupRect { key: string; label: string; color: string; value: number; x: number; y: number; w: number; h: number; showHead: boolean }

// nested squarified treemap: outer rects per group (area ∝ group value), inner tiles per holding
function groupedTreemap(tiles: SqItem[], mode: TmapMode, lang: Lang, W: number, H: number): { groupRects: GroupRect[]; tiles: PlacedTile[] } {
  const map: Record<string, TmapGroup & { items: SqItem[]; value: number }> = {};
  tiles.forEach((t) => {
  ...
  const outer = squarifyTreemap(groups.map((g) => ({ g, value: g.value } as unknown as SqItem)), W, H) as unknown as (PlacedTile & { g: TmapGroup & { value: number } })[];
  const HEAD = 16, GAP = 3;
  const groupRects: GroupRect[] = [], placed: PlacedTile[] = [];
  outer.forEach(({ g, x, y, w, h }) => {
    ...
    const grp = map[g.key];
    const inner = squarifyTreemap(grp.items.slice().sort((a, b) => b.value - a.value), gw, ih);
...
function PortfolioTreemap({ tiles, lang, onOpen }: { tiles: SqItem[]; lang: Lang; onOpen: (p: UIPlan, tab?: string) => void }) {
  const hko = lang === "ko";
  const ref = useRef<HTMLDivElement>(null);
  const [w, setW] = useState(0);
  const [mode, setMode] = useState<TmapMode>(() => { try { return (localStorage.getItem(TMAP_LSK) as TmapMode) || "none"; } catch { return "none"; } });
  const setG = (m: TmapMode) => { setMode(m); try { localStorage.setItem(TMAP_LSK, m); } catch { /* SSR/quota */ } };
  useLayoutEffect(() => {
  ...
  const result = (w > 0)
    ? (grouped ? groupedTreemap(tiles, mode, lang, w, H) : { groupRects: [] as GroupRect[], tiles: squarifyTreemap(tiles, w, H) })
    : { groupRects: [] as GroupRect[], tiles: [] as PlacedTile[] };
  const modes: [TmapMode, string][] = [["none", hko ? "없음" : "None"], ["portfolio", hko ? "포트폴리오" : "Portfolio"], ["sector", hko ? "섹터" : "Sector"], ["market", hko ? "시장" : "Market"]];
  return (
      ...
      <div className="dash-tmap" ref={ref} style={{ height: H }}>
        {result.groupRects.map((g) => (
          <div className="dash-tmap-grp" key={"g_" + g.key} style={{ left: g.x, top: g.y, width: g.w, height: g.h }}>
            {g.showHead && <span className="dash-tmap-glabel">{mode === "market" ? <Flag market={g.key as "KR" | "US"} size={11} /> : mode === "portfolio" ? <span className="pf-dot" style={{ background: g.color, width: 7, height: 7 }} /> : null}{g.label}</span>}
          </div>
        ))}
        {result.tiles.map(({ p, rate, x, y, w: tw, h: th }) => {
          ...
          return (
            <div className="dash-tmap-tile" key={p.id} onClick={() => onOpen(p)}
              ...
              title={p.tickerName[lang] + (tag ? " · " + tag : "") + " · " + (rate >= 0 ? "+" : "") + rate.toFixed(1) + "%"}>
...
function DashRow({ plan, t, lang, onOpen }: { plan: UIPlan; t: I18nDict; lang: Lang; onOpen: (p: UIPlan, tab?: string) => void }) {
  ...
  const fill = plan.divisions ? Math.max(0, Math.min(1, (plan.round ?? 0) / plan.divisions)) : null;
    ...
    <div className="dash-row" onClick={() => onOpen(plan, "executions")}>
      <div className="dash-c dash-name">
        ...
        <span className="dash-nm">
          <span className="dash-nm-title"><Flag market={plan.cur === "KRW" ? "KR" : "US"} size={12} /> {plan.tickerName[lang]}{(() => { const tg = planTag(plan, lang); return tg ? <><span className="dash-nm-sep">·</span><span className="dash-nm-plan">{tg}</span></> : null; })()}</span>
      ...
      <div className="dash-c dash-price">
        {hasPos ? <>
          ...
          <span className="mono">{fmtCompact(plan.currentPrice, plan.cur)}</span>
        </> : <span className="dim">{t.dash_watching}</span>}
      </div>
      <div className="dash-c dash-pl">
        {ret ? <>
          <span className={"dash-pl-pct mono " + (ret.rate >= 0 ? "pos" : "neg")}>{ret.rate >= 0 ? "+" : ""}{ret.rate.toFixed(1)}%</span>
...
type RefKey = "bear" | "base" | "bull";
const REF_EN: Record<RefKey, string> = { bear: "Bear", base: "Base", bull: "Bull" };
const REF_KO: Record<RefKey, string> = { bear: "하단", base: "중간", bull: "상단" };
const REF_COLOR: Record<RefKey, string> = { bear: "var(--r-bear)", base: "var(--r-base)", bull: "var(--r-bull)" };
const REF_FULL: Record<string, string> = { bear: "하단 목표가", base: "중간 목표가", bull: "상단 목표가" };
function refTarget(p: UIPlan, ref: RefKey): number | null {
  const s = (p.scenarios || []).find((x) => x.label && x.label.en === REF_EN[ref]);
  if (s && s.target != null) return s.target;
  return ref === "base" ? (p.iv ?? null) : null;
}

export function DashboardView({ plans, t, lang, onOpen, grouping = "portfolio", ordering = "updated" }: {
  plans: UIPlan[]; t: I18nDict; lang: Lang; onOpen: (p: UIPlan, tab?: string) => void;
  grouping?: string; ordering?: Ordering;
}) {
  const [buyRef, setBuyRef] = useState<RefKey>(() => { try { return (localStorage.getItem("ks_actq_buyRef") as RefKey) || "base"; } catch { return "base"; } });
  const [sellRef, setSellRef] = useState<RefKey>(() => { try { return (localStorage.getItem("ks_actq_sellRef") as RefKey) || "bull"; } catch { return "bull"; } });
  const pickBuy = (v: RefKey) => { setBuyRef(v); try { localStorage.setItem("ks_actq_buyRef", v); } catch { /* SSR/quota */ } };
  const pickSell = (v: RefKey) => { setSellRef(v); try { localStorage.setItem("ks_actq_sellRef", v); } catch { /* SSR/quota */ } };
  const [band, setBand] = useState<number>(() => { try { const n = parseInt(localStorage.getItem("ks_actq_band") || "8", 10); return isNaN(n) ? 8 : n; } catch { return 8; } });
  const pickBand = (v: number) => { const n = Math.max(1, Math.min(50, Math.round(v))); setBand(n); try { localStorage.setItem("ks_actq_band", String(n)); } catch { /* SSR/quota */ } };
  const [bandEditing, setBandEditing] = useState(false);
  const [bandDraft, setBandDraft] = useState("");
  const sortItems = (arr: UIPlan[]) => orderPlans(arr, ordering, lang);

  let groups: { key: string; label: string | null; dot?: string; icon?: React.ReactNode; items: UIPlan[] }[];
  if (grouping === "status") {
  ...
  const ko = lang === "ko";
  const NEAR = band / 100;
  const acts: { p: UIPlan; kind: "buy" | "sell"; ref: RefKey; target: number }[] = [];
  plans.forEach((p) => {
    ...
    <div className="dash">
      <div className="dash-headline">
        <DashStat lab={t.dash_open} val={String(openN)}
          tip={plans.filter((p) => ["active", "paused", "closing"].includes(p.status)).map((p) => { const ex = EXEC_STRATEGIES.find((s) => s.id === p.execId); return { name: tipName(p, lang), flag: <Flag market={p.cur === "KRW" ? "KR" : "US"} size={11} />, val: ex ? <span className="dash-stat-tip-strat"><span className="ref-dot" style={{ background: ex.color }} />{ex.name[lang]}</span> : "—" }; })} />
        ...
        {acts.length > 0 && <DashStat lab={t.dash_actionq} val={String(acts.length)} tone="warn"
          tip={acts.map(({ p, kind }) => ({ name: tipName(p, lang), flag: <Flag market={p.cur === "KRW" ? "KR" : "US"} size={11} />, val: <span className="dash-stat-tip-strat"><span className="ref-dot" style={{ background: kind === "buy" ? "var(--pos)" : "var(--neg)" }} />{kind === "buy" ? (ko ? "매수 검토" : "Buy") : (ko ? "매도 검토" : "Sell")}</span> }))} />}
      </div>

      {posPlans.length > 0 && (() => {
        const tiles: SqItem[] = posPlans.map((p) => ({ p, val: toKRW(p.currentPrice * p.totalShares, p.cur), value: toKRW(p.currentPrice * p.totalShares, p.cur), rate: (planReturn(p) || { rate: 0 }).rate }))
        ...
        <div className="dash-actq-head">
          ...
          {ko && <div className="dash-actq-sentence">
            ...
            <MiniDropdown width={196} align="left"
              ...
              items={([["bear", "보수적 — 더 싸질 때만"], ["base", "적정가 부근에서 매수"]] as [RefKey, string][]).map(([v, d]) => ({ value: v, label: REF_FULL[v], sub: d, on: buyRef === v, icon: <span className="ref-dot" style={{ background: REF_COLOR[v] }} /> }))}
              onPick={(v) => pickBuy(v as RefKey)} />
            ...
            <MiniDropdown width={196} align="left"
              ...
              items={([["base", "적정가 도달 시 검토"], ["bull", "낙관 목표 도달 시 검토"]] as [RefKey, string][]).map(([v, d]) => ({ value: v, label: REF_FULL[v], sub: d, on: sellRef === v, icon: <span className="ref-dot" style={{ background: REF_COLOR[v] }} /> }))}
              onPick={(v) => pickSell(v as RefKey)} />
          ...
          {acts.map(({ p, kind, ref, target }) => {
            ...
            return (
              <div className={"dash-act dash-act--" + kind} key={p.id} onClick={() => onOpen(p, "executions")}>
                ...
                <span className="dash-act-name">{p.tickerName[lang]}{(() => { const tg = planTag(p, lang); return tg ? <><span className="dash-act-sep">·</span><span className="dash-act-plan">{tg}</span></> : null; })()}</span>
