...
function calcValuation(plan, inp) {
  ...
  const up = (v) => price > 0 ? (v / price - 1) * 100 : 0;
...
function fairToScenarioTargets(r) {
...
function histMultipleBand(plan, type) {
...
function seedBands(plan) {
  const r = calcValuation(plan, seedFinancials(plan));
  const r1 = (v, d) => { const x = Math.round(v * 10) / 10; return x > 0 ? x : d; };
  const band = (type, fb) => { const h = histMultipleBand(plan, type); return (h && h.lo > 0 && h.hi > h.lo) ? h : fb; };
...
function bandTargets(r, mode, band) {
...
function seedSlots(plan, kind) {
  const valid = ["per", "pbr", "psr", "ev", "fcf", "por"];
  const k = valid.includes(kind) ? kind : "per";
  const bb = seedBands(plan)[k];
  return [{ m: k, v: bb.lo }, { m: k, v: bb.mid }, { m: k, v: bb.hi }];
}

Object.assign(window, { valFactor, seedFinancials, calcValuation, reverseFromPer, reverseFromCap, fairToScenarioTargets, seedBands, bandTargets, seedSlots });
