...
3. ✅ **경로(ex3) 정식 오버레이 완료(2026-07-05 2차, 옵션2)** — 밸류애버리징 전용 `isVA` 콕핏 오버레이(가치경로 밴드차트, isVR 구조 미러) + path 실발동. **VA 모델은 골든 sim(`futuretest.jsx:114-121` value kind) 미러**: `desired=target_path×(round+1)`(월 목표증가액×회차), `band=±4%×step`, 평가액 경로 아래/위(>4%)면 부족분 매수/초과분 매도. `rule-eval-v2` path_gap을 항상-armed → **실발동(fired)**으로 교체. `strategy-tab`에 isVA 감지(isPrice에서 target_path 제외) + 오버레이 + next-action. E2E: PLN-001 ex3(60주/12회차)→목표 ₩7.8M·갭 +₩9.36M·"경로 초과·매도" fired 확인, tsc0·골든102·콘솔0·core무수정. ⚠️ 극단값(평가액≫경로)에선 now마커 우측 클램프(isVR/isWeight와 동일 거동) — 실 VA 플랜(경로 근처)에선 정상.
