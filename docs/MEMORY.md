...
| 7 | 웹 이식 (6보다 선행 결정) | 🔄 2026-07-03 — **원본 6개 스크린(01~06) 전부 완료**: Auth/온보딩/앱셸 · 03 리스트+사이드바 · 04 상세(8탭+우측바) · 01 인박스(체결 DB왕복) · 02 일지(노트 피드) · 05 전략편집기(읽기전용 4탭). **07 대시보드·16 인사이트·10 시나리오모니터·19~22 종목상세 완료(2026-07-04)** — 07=`dashboard-view.tsx`. 16=`insights/insights-screen.tsx`. 10=`scenarios/scenarios-screen.tsx`(+공용 FilterPanel). 19~22=`securities/[ticker]/`+`components/securities/security-detail.tsx`(헤더·차트·계절성·4탭[재무/투자지표/밸류=기존 탭 secPlan 재사용]·관심토글 서버액션). 남은 것 = **11~13 스크리너·14 관심종목·15 리서치·17 보관함·18 휴지통**(watchlist/screener/research는 이제 onOpenSecurity→`/securities/[ticker]` 가능=언블록됨). ✅ source/core 재조정은 실측 no-op으로 종결(2026-07-04, 라인엔딩 차이뿐) |
...
- **19~22 종목상세(SecurityDetail) 이식 이음새(2026-07-04)**: `lib/security-mapper.ts`(mapSecurity→UISecurity + secPlanOf→합성 UIPlan + SECURITY_SELECT/SECURITY_FIN_SELECT) · `app/(shell)/securities/[ticker]/`(page 서버페치+actions.ts `toggleWatch` 서버액션) · `components/securities/security-detail.tsx`(SecurityChart·SeasonalityHeatmap·SecurityDetailScreen). **핵심 = 재무/투자지표(21)/밸류에이션(22) 탭은 기존 FinancialsTab/IndicatorsTab/ValuationTab을 `plan={secPlan} fin={fin}`으로 재사용**(plans/[id] 패턴 동일, fin=buildPlanFin). **관심토글**: watchlist 테이블(user_id,ticker unique) insert/delete + 낙관적 useState. **⚠️ eps 유도**: securities/security_financials엔 eps 없음 → **같은 ticker 플랜에서 유도**(`plans.find(p=>p.ticker===tk && p.eps>0)?.eps`, updated_at desc라 최신 플랜) → PER/EPS/밸류에이션 실값. 플랜 없는 종목은 eps=0 폴백(seam). **defer**: 종목 메모 CRUD(notes 테이블 없음)·SecurityScenarios(adhoc)·SecurityPeek 팝오버·Cmd+K SearchModal·onCreatePlan/onAddScenario·`change` 일일%(DB 미저장 mock 0)·spark(genSpark mock). 라우트 진입=`/securities/[ticker]`(watchlist/research/screener onOpenSecurity 대상)
