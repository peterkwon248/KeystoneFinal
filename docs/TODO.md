...
- [x] **02 일지(Journal)** — 3-pane 노트 피드 (2026-07-03, 브라우저 E2E). 플랜 노트 기반, patchNotes 재사용, since 트랙(jrWritePrice mock). 종목 저널은 종목상세 이식 때 후속
- [x] **05 전략 편집기(StrategyEditor)** — 읽기전용 4탭(개요/필드/규칙/미리보기), 관점(등급룰)·전략(WHEN→THEN) 분기 (2026-07-03, 브라우저 E2E). core 프리셋, mutation 0
- [x] **원본 6개 스크린(01~06) 전부 완료** (06 청산 = 인박스 리더 청산카드로 커버)
- [ ] 새 핸드오프 스크린(screens 6→22): **07 대시보드(현황)** ← 다음 / 10 시나리오모니터 / 11~13 스크리너 / 14 관심종목 / 15 리서치 / 16 인사이트 / 17 보관함 / 18 휴지통 / 19~22 종목상세
