...
- [~] **04 플랜 상세(DetailView) — 8탭 중 7개 완료** (2026-07-03, 브라우저 E2E + 골든 102):
  - [x] 셸 · [x] 시나리오(카드·수렴분석·GapTab 차트) · [x] 활동 · [x] 체결(장부·성과밴드) · [x] 재무제표(IS/BS/CF 실연결)
  - [x] 전략(콕핏 6종 오버레이·룰카드·룰토글/목표 뮤테이션) · [x] 투자지표(5뷰모드·프레임워크 렌즈) · [x] 밸류에이션(적정가 계산기·민감도·역산·밴드차트2종·적용→시나리오 뮤테이션)
  - [ ] **인사이트(planinsights.jsx)** ← 마지막 8번째 탭
  - [ ] **우측 디테일바(PropsSidebar, `.detail-side`)** ← 크로스탭 우측레일, 인사이트 뒤 별도 증분 (포지션/청산요약·전략파라미터·커스텀필드·노트+접기토글)
