...
> 최종 갱신: **시나리오 차트 마무리 라운드(m2130~m2210).** 완료: 색 정리(상단 초록·중간 노랑·하단 빨강·추정 회색점선·시장가 흰선), 축 아래 이벤트 틱(일지=연필/룰=번개 아이콘, 팝오버 토글↔범례 연동, 각 범례 호버툴팁), 최근범위 리치 호버툴팁(52주/전체 고·저+날짜+현재가+고점/저점대비), 표시 팝오버(상/중/하단·추정·시장가 강제표시 = on/off 삭제, 목표선 실선/점선/굵기 스타일만 + 마커 토글), 계단 꺾임점에 "목표가 수정" 마커(scenarioHistory 있는 플랜), 범례 다이아몬드 아이콘을 차트 마커와 정합.

## ✅ 최근 완료 (이번 세션) — 밸류에이션 차트 · 계절성 히트맵 · 크로스뷰 필터
...
## 5. 남은 작업 (사실상 이것 하나)
- **API 연동 인프라 (클로드코드 핸드오프)** — DART / SEC EDGAR / Finnhub 등 실제 fetch 어댑터. 구조는 이미 잡혀 있음: 라이브/재연결 상태(`conn`/`reconnect`), 한·미 재무제표 스키마 분기(K-IFRS vs US-GAAP, `plan.cur`로 판정), mock FX(`KEYSTONE_FX`) → 라이브 환율 교체 지점. **실제 네트워크 연결만 남음.** 백엔드 붙이기 전 "원형 완성"이 목표였고 여기 도달함.
